...
fn latest_episode_airdate(episodes: &[Episode]) -> Option<String> {
    let ep = episodes.iter().filter(|e| e.item_type == 0).max_by(|a, b| {
        a.sort.partial_cmp(&b.sort).unwrap_or(std::cmp::Ordering::Equal)
    });
    ep.map(|e| e.airdate.clone()).filter(|d| !d.is_empty())
}

fn within_window(date: Option<NaiveDate>, window_start: NaiveDate) -> bool {
    match date { Some(d) => d >= window_start, None => false }
}

fn is_finished(expected_eps: Option<u32>, current_eps: Option<u32>) -> bool {
    match (expected_eps, current_eps) { (Some(exp), Some(cur)) if exp > 0 && cur >= exp => true, _ => false }
}

fn determine_code(first_air: Option<NaiveDate>, latest_air: Option<NaiveDate>, calendar_on_air: bool, finished: bool, today: NaiveDate, window_start: NaiveDate) -> SubjectStatusCode {
    match first_air {
        Some(fa) if fa > today => SubjectStatusCode::PreAir,
        Some(_) => {
            if calendar_on_air { SubjectStatusCode::Airing } else if finished { SubjectStatusCode::Finished } else if within_window(latest_air, window_start) { SubjectStatusCode::Airing } else { SubjectStatusCode::OnHiatus }
        }
        None => {
            if calendar_on_air { SubjectStatusCode::Airing } else if finished { SubjectStatusCode::Finished } else if within_window(latest_air, window_start) { SubjectStatusCode::Airing } else { SubjectStatusCode::Unknown }
        }
    }
}
...
fn status_reason(code: &SubjectStatusCode, calendar_on_air: bool) -> String {
    match code {
        ...
        SubjectStatusCode::Airing => { if calendar_on_air { "当周日历在播".to_string() } else { "最近三周有更新".to_string() } }
...
pub async fn calc_subject_status(id: u32) -> Result<SubjectStatus, AppError> {
    ...
    let calendar_on_air = calendar.iter().any(|day| day.items.iter().any(|item| item.id == id));
    ...
    if current_eps.is_none() { current_eps = Some(initial_eps.total); }
    if initial_eps.total > 0 {
        ...
        let last_page = if offset_last == initial_eps.offset { initial_eps } else { fetch_episodes(id, None, Some(limit), Some(offset_last)).await? };
        latest_airdate = latest_episode_airdate(&last_page.data);
    }
    let today = Utc::now().date_naive();
    let window_start = today.checked_sub_days(Days::new(RECENT_WINDOW_DAYS)).unwrap_or(today);
    ...
    let code = determine_code(first_air, latest_air, calendar_on_air, finished, today, window_start);
    let reason = status_reason(&code, calendar_on_air);
    Ok(SubjectStatus { code, first_air_date, latest_airdate, expected_eps, current_eps, calendar_on_air, reason })
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::services::bangumi_service::api::{fetch_subject as fsub, search_subject as ssub, fetch_episodes as feps};
    ...
    async fn test_search_subject() {
        let res = ssub("Fate", Some(vec![2]), None, None, None, None, None, None, None, Some(10), Some(0)).await.unwrap();
        assert!(res.total > 0);
    ...
    async fn test_calc_subject_status_returns_any_code() {
        ...
        match res.code {
            SubjectStatusCode::PreAir | SubjectStatusCode::Airing | SubjectStatusCode::Finished | SubjectStatusCode::OnHiatus | SubjectStatusCode::Unknown => {}
        }
    }
}
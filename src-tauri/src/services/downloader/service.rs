...
impl DownloaderService {
    ...
    pub async fn add_task(&self, anime_id: i64, episode_id: i64, magnet: String, save_path: String, metadata: DownloadTaskMetadata) -> Result<i64, AppError> {
        // 1. Call rqbit
    ...
    pub async fn list_tasks(&self) -> Result<Vec<serde_json::Value>, AppError> {
         ...
         for task in db_tasks {
             let metadata: DownloadTaskMetadata = serde_json::from_str(&task.metadata).unwrap_or_else(|_| DownloadTaskMetadata {
        ...
        tauri::async_runtime::spawn(async move {
            loop {
                if let Err(_e) = Self::sync_once(&client, &app).await {
                    // Fail silently to avoid spamming logs when sidecar is down
                }
    ...
    async fn sync_once(client: &RqbitClient, app: &AppHandle) -> Result<(), AppError> {
        ...
        for db_task in db_tasks {
            if let Some(r_task) = rqbit_tasks.iter().find(|t| t.info_hash == db_task.info_hash) {
                let new_status = r_task.state.as_deref().unwrap_or("unknown");
                if db_task.status != new_status {
                    repo::update_status(db_task.info_hash.clone(), new_status.to_string()).await.ok();
                }
                
                let metadata: DownloadTaskMetadata = serde_json::from_str(&db_task.metadata).unwrap_or_else(|_| DownloadTaskMetadata {
                    anime_title: "Unknown".into(),
                    episode_title: "Unknown".into(),
                    image_url: None,
                });
